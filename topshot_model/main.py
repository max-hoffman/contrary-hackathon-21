...
def get_data_from_url(url):
    ...
    # COUNTS
    moment["num"] = int(
        soup.find("span", {"class": "Heading__H3-kksint-2 iZrhpZ"}).text
    )
    collectorsDiv = soup.find(
        "div", {"class": "UserStockAndSeller__StyledRoot-sc-1sq21bh-0 jZrKdp"}
    )
    moment["numCollectors"] = int(
        collectorsDiv.find(
            "span",
            {
                "class": "Text-sc-179eaht-0 UserStockAndSeller__StyledText-sc-1sq21bh-4 btAjbQ"
            },
        ).text.split()[0]
    )
    moment["numSale"] = int(
        collectorsDiv.find(
            "span",
            {
                "class": "Heading__H4-kksint-3 UserStockAndSeller__Count-sc-1sq21bh-1 hQzWLl hCJKtG"
            },
        ).text
    )
    ...
    playerGameScores = playerGameScoresTable.find_all(
        "td", {"class": "Label-sc-1c0wex9-0 Tablestyles__Cell-xih7iu-0 ebXDLw fNBBpY"}
    )
    moment["plusMinusGame"] = float(playerGameScores[-1].text)

    playerSeasonAverageScoresTable = soup.find(
        "table", {"id": "playerSeasonAverageScores"}
    )
    playerSeasonAverageScores = playerSeasonAverageScoresTable.find_all(
        "td", {"class": "Label-sc-1c0wex9-0 Tablestyles__Cell-xih7iu-0 ebXDLw fNBBpY"}
    )
    moment["plusMinusSeason"] = float(playerSeasonAverageScores[-1].text)

    # IMAGE
    moment["img"] = soup.find("img", {"class": "Gallery__Img-sc-1rdwc7w-2 bzthkn"})[
        "src"
    ][:-21]
...
def format_row(data):
    values = [
        ...
        data["num"],
        data["numCollectors"],
        data["plusMinusSeason"],
        data["action_3 Pointer"],
        data["action_Dunk"],
        data["action_Handles"],
        data["action_Jump Shot"],
        data["action_Layup"],
    ]
    return values
...
def load_model():
    path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "MLR_model_v2.pickle"
    )
